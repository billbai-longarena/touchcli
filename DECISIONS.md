...
- `.termite.db` 当前无 `open/claimed/parked` 信号；现存信号均为 `completed` 或 `archived`。
- 协议脚本已修正为“活跃集合排除 `completed/done`”，心跳不会再误选已完成任务。

### 决策动作

1. 白蚁心跳在“活跃队列=0”时进入**监测模式**：执行 arrive/sense + pheromone 沉积，不重复关闭已完成信号。
2. 仅当出现新 `open` 信号、`ALARM.md`、或明确指令通道任务时，转回执行模式（claim/work/release）。
3. `BLACKBOARD.md` 作为叙述层信息，不作为运行态真值；运行态一律以 `.termite.db` 为准。
